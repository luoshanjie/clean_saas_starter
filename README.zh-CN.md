...
- 内核应稳定在“能力边界”上，而不是稳定在某个具体 SDK 或实现版本上
- 核心业务和用例层不应直接依赖数据库驱动、对象存储 SDK 或云厂商特定 API
- Go、Echo、pgx、MinIO SDK、Casbin 等底层组件升级，应该优先由 adapter 和 bootstrap 层吸收
- 如果新版运行时或依赖能自然带来性能、稳定性或安全性收益，脚手架应尽量保持可升级，以便直接获得这些收益
- 基础设施差异可以存在，但必须明确写进文档，而不是伪装成完全等价
